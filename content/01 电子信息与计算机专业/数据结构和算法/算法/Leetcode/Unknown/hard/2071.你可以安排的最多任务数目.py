...
class Solution:
    def maxTaskAssign(self, tasks: List[int], workers: List[int], pills: int, strength: int) -> int:
        ...
        def canComplete(k:int)->bool:
            ...
            for i in range(k-1,-1,-1):# 倒序，最大的任务开始分配
                if tasks[i]<=available_workers[-1]:
                    available_workers.pop()
                elif available_pills>0:
                    # 找到最弱且能在使用药丸后完成任务的工人
                    idx = bisect.bisect_left(available_workers, tasks[i]-strength)
        ...
        while left<=right:
            mid = (left+right)//2 # 这里必须要加1，向上取整，否则可能mid一直等于left，出现死循环
            if canComplete(mid):
                left = mid + 1
            else:
                right = mid-1
