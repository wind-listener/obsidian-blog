...
1. **不满足三角不等式**
2. **加法性（Additivity）**：如果我们有多个分布，可以通过加法将它们的KL散度求和。假设有三个概率分布$P$, $Q$, $R$，那么: $D_{\text{KL}}(P \parallel Q) + D_{\text{KL}}(Q \parallel R) = D_{\text{KL}}(P \parallel R)$

3. **与熵的关系**：KL散度与信息论中的**熵**（Entropy）有着紧密的联系。假设$P$和$Q$是两个概率分布，则KL散度可以分解为：$D_{\text{KL}}(P \parallel Q)=H(P, Q)-H(P)$
